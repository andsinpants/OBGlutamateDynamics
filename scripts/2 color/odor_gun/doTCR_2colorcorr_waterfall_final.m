...
%             
        ...
        elseif tmpmin<minthresh %only suppressed
            ...
            results2(index,2)=odornum;
            results2(index,1)=r;
            results_trace(index,1:length(supzts))=supzts; %<<<<<<<<<<<<< extract time series (sup)
            results_ts(index,1)=r; 
            results_ts(index,2)=odornum;
...
%% channel 2 sorting into biphasic, excitatory, suppressive responses
       if tmpmax2>maxthresh && tmpmin2<minthresh %plot both excitatory and suppressive
            ...
            results_ts2(index,1)=r; results_ts2(index,2)= odornum; results_ts2(index,3)=tmpmax2; results_ts2(index,4)=tmpmin2;
            if bplotbyodor
        ...
        elseif tmpmin2<minthresh %only suppressed
            ...
            results2(index,2)= odornum; 
            results2(index,1)=r;
            results_trace2(index,1:length(supzts2))=supzts2;   %<<<<<<<<<<< extract time series (biphasic)
            results_ts2(index,1)=r; 
            results_ts2(index,2)=odornum; 
...
% for i=1:size(results_sig2_tbt82,1)
...
for i=1
    if tf == 1
       save ('results_timeseries.mat', 'results_sig', 'results_sig2')
    else
        break
    end
end
