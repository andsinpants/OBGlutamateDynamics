...
supsigrois = allplot_smth(goodspans_sup,:);
%excroisz = zdata_odorsmth(goodspans,:)
...
prompt = ('Is this a Ch1 or Ch2 avgfile? 1/2?');
savequestion = input(prompt,'s');
tf1='1';
tf2='2';
chcmp=strcmpi(savequestion,'1');

for k =1
    if tf==1 && chcmp==1
          for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuproiunsorted_ch1.mat', 'excsigrois')
             elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuproisunsorted_ch1.mat', 'excsigrois', 'supsigrois')
            end
        end

        for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuprois_sorted_ch1.mat', 'sorted_excplotsmth')
            elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuprois_sorted_ch1.mat', 'sorted_excplotsmth', 'sorted_supplotsmth')
            end
        end

        for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuprois_sortandnorm_ch1.mat', 'normexc')
            elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuprois_sortandnorm_ch1.mat', 'normexc','normsup')
            end
        end

        for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuprois_ch1.mat', 'goodspans_exc')
            elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuprois_ch1.mat', 'goodspans_exc', 'goodspans_sup')
            end
        end
    ...
    elseif tf==1 && chcmp==0
        for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuproiunsorted_ch2.mat', 'excsigrois')
             elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuproisunsorted_ch2.mat', 'excsigrois', 'supsigrois')
            end
        end

        for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuprois_sorted_ch2.mat', 'sorted_excplotsmth')
            elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuprois_sorted_ch2.mat', 'sorted_excplotsmth', 'sorted_supplotsmth')
            end
        end

        for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuprois_sortandnorm_ch2.mat', 'normexc')
            elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuprois_sortandnorm_ch2.mat', 'normexc','normsup')
            end
        end

        for i=1
            if ~exist('supsigrois','var') || isempty(supsigrois)
                save ('excandsuprois_ch2.mat', 'goodspans_exc')
            elseif tf==1 && ~isempty(supsigrois)
                save ('excandsuprois_ch2.mat', 'goodspans_exc', 'goodspans_sup')
            end
        end
    else
        break
