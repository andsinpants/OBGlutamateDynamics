...
decayCh150=[];
count=0;
todoron=581;  %hardcode in time of stim on in frames (150 Hz pseudoframe rate)
duration=200;   %this used to find max and look for onset latency. Not for FWHM.
...
numrois=length(myplotdata.avgfile.roi);
for j=1:numrois
    %for i = 1:numfiles
        count=count+1;
        %allresults.trial(count).filename=myplotdata.file(i).name;
        allresults.roi(count)=myplotdata.file(1).roi(j).number;
        %allresults.trial(count).odorname=myplotdata.file(i).type;  %change this later to be odorname
        trace=myplotdata.avgfile.roi(j).odor.avgtrial.series;  %pull out trace in question
       ...
       % prenoise=std(trace(todoron-preodorduration:todoron)); %noise of baseline
        ...
        if latencysd_in
            allresults.trial(count).onsetlat=latencysd_in;  % note will be empty if never gets above threshold for long enough
        ...
        %now Shaina's code to do smoothing and find peak times and FWHM
             ysmooth2 = smoothdata(trace(1:todoroff),'gaussian',40);   %what is this width? Assume that width = 6sigma, so, length of 40 = 8.3. which = sigma of 45 msec at .067 frame interval
        ...
        allresults.trial(count).peaktime=peakTime;
