...
[wshoutfile,wshoutpath] = uigetfile('*.mat','Choose Washout file','MultiSelect','off');
if ~wshoutfile; return; end
wshoutdrug = importdata(fullfile(wshoutpath,wshoutfile)); clear wshoutpath wshoutfile;
...
for r=1:length(wshoutdrug.file.roi)
wshoutdrug.data(r,:) = wshoutdrug.file.roi(r).odor.avgtrial.series';
end
...
%% now same sorting and plotting of washout condition data (1st sniff)
clearvars exzts exbasemean exbasemeansub exbasemeanll exbasestd tmpmax supzts supbasemean supbasemeansub supbasemeanll supbasestd tmpmin 
index=0;

for r = 1:size(wshoutdrug.data,1)
     for o = 1:length(1)
        exbasemean = mean(wshoutdrug.data(r,ibase));
        exbasemeansub = cgpdrug.data(r,ibase)-exbasemean;
        exbasemeanall = vertcat(exbasemeansub,exbasemeanall);  
        
        supbasemean = mean(wshoutdrug.data(r,ibase));
        supbasemeansub = wshoutdrug.data(r,ibase)-supbasemean;
        supbasemeanall = vertcat(supbasemeansub,supbasemeanall);
    ...
    for o = 1:length(1)
        odornum = 1;
        exbasemean = mean(wshoutdrug.data(r,ibase(1):ibase(end)));
        exzts = (wshoutdrug.data(r,:)-exbasemean)./exbasestd(1,r);
%       tmpmax = prctile(exzts(130:135),95);
        tmpmean = mean(exzts(iodor(1):iodor(end)));
%       tmpmax = tmpmean;
        tmpmax = prctile(exzts(iodor),95);
        supbasemean = mean(wshoutdrug.data(r,ibase(1):ibase(end)));
        %supbasestd = std(sup.myplotdata.avgfile.roi(r).odor(o).avgtrial.series(ibase));
        supzts = (wshoutdrug.data(r,:)-supbasemean)./supbasestd(1,r);
        tmpmin = prctile(supzts(iodor),15); %default is 25
        ...
        if  k(r) == 1 %only excited
           results_wshout(r,odornum,1)=tmpmax;
           results_trace_wshout(index,1:length(exzts))=wshoutdrug.data(r,:);  %<<<<<<<<<<< extract time series (exc)
           results_ts_wshout(index,2)=odornum;
           results_ts_wshout(index,1)=r;
           results_ts_wshout(index,3)=tmpmax; 
           results_ts_wshout(index,4)=0;
            if bplotbyodor 
                figure(odornum); hold on;
            ...
            plot(times,exzts,'Color',tmpcolor,'LineStyle','-','DisplayName',tmplabel);
            figure(256);
            plot(3,results_wshout(r,odornum,1),'x','Color','r');
        elseif p(r) == 1 %only suppressed
           results_wshout(r,odornum,2)=tmpmin; 
           results_trace_wshout(index,1:length(supzts))=wshoutdrug.data(r,:); %<<<<<<<<<<<<< extract time series (sup)
           results_ts_wshout(index,2)=odornum;
           results_ts_wshout(index,1)=r;
           results_ts_wshout(index,3)=0; 
           results_ts_wshout(index,4)=tmpmin;
            if bplotbyodor
                figure(odornum); hold on;
            ...
            plot(times,supzts,'Color',tmpcolor,'LineStyle','-','DisplayName',tmplabel);
            figure(256);
            plot(3,results_wshout(r,odornum,2),'x','Color','b');
        else
           results_wshout(r,odornum,2)=0; 
           results_trace_wshout(index,1:length(exzts))=0; 
           results_ts_wshout(index,2)=odornum;
           results_ts_wshout(index,1)=r;
           results_ts_wshout(index,3)=0; 
           results_ts_wshout(index,4)=0;
...
figure;
for i = 1:size(wshoutdrug.data,1)
    firstsniff_wshoutdrug(i) = mean(results_trace_wshout(i,iodor))-mean(results_trace_wshout(i,ibase));
    secondsniff_wshoutdrug(i) = mean(results_trace_wshout(i,iodor2))-mean(results_trace_wshout(i,ibase2));
    secandfirsniff_wshoutdiff(i) = secondsniff_wshoutdrug(i)-firstsniff_wshoutdrug(i);
    concatsniff_wshoutdrug = vertcat(firstsniff_wshoutdrug,secondsniff_wshoutdrug);
    j(i) = concatsniff_wshoutdrug(1,i)~= 0 && concatsniff_wshoutdrug(2,i)~= 0; 
    if j(i) == 1
    plot([1 2],[firstsniff_wshoutdrug(i) secondsniff_wshoutdrug(i)],'--o');hold on;title('Sniff 1 and All Odor Mean (Washout)');xlabel('Sniff 1 || All Odor Mean');ylabel('{\Delta}F/F');
    end
end
...
for i = 1:size(pdrug.data,1)
    j(i) = concatsniff_pdrug(1,i)~= 0 && concatsniff_pdrug(2,i)~= 0;
    if j(i) == 1
        plot([1 2 3],[secandfirsniff_pdiff(i) secandfirsniff_cgpdiff(i) secandfirsniff_wshoutdiff(i)],'--x');hold on;title('{\Delta}F All Odor Mean - {\Delta}F Sniff 1 (Across Conditions)');xlabel('Condition');ylabel('Change in {\Delta}F/F');
    end
end
...
%     figure(i);line([1,size(temptrace_cgpdrug,2)],[LL_cgpdrug, LL_cgpdrug],'Color','blue','LineWidth',2);line([1,size(temptrace_cgpdrug,2)],[mean_pre, mean_pre],'Color','green','LineWidth',2)
    ...
    %temptrace_wshout=[]; R = []; peakamp1 = []; %peakindex = []; %clearvars in loop
    [peakampl, peakindex]=max(results_trace_wshout(i,iodor));
    peaklat_wshout(i)=peakindex*(64.5/uf);
    temptrace_wshout=results_trace_wshout(i,iodor);
    mean_pre=mean(temptrace_wshout(1:25));
    %     figure(i);plot(temptrace_wshout);hold on;
    temptrace_wshout(peakindex:end)=peakampl;
    figure(i);plot(temptrace_wshout);
    [R,LT_wshout,UT_wshout,LL_wshout,UL_wshout]=risetime(temptrace_wshout,(15.5*uf),'StateLevels',[mean_pre peakampl]);
     figure(i);line([1,size(temptrace_wshout,2)],[UL_wshout, UL_wshout],'Color','cyan','LineWidth',2);line([1,size(temptrace_wshout,2)],[mean_pre, mean_pre],'Color','yellow','LineWidth',2);hold off
%     figure(i);line([1,size(temptrace_wshout,2)],[LL_wshout, LL_wshout],'Color','cyan','LineWidth',2);line([1,size(temptrace_wshout,2)],[mean_pre, mean_pre],'Color','yellow','LineWidth',2);hold off
    if isempty(LT_wshout) || isempty(UT_wshout)
        LT_wshout = 0
        UT_wshout = 0
    end
    onsets_wshout(i)=LT_wshout(1)*1000;  %convert seconds to milliseconds
    peaklats90_wshout(i)=UT_wshout(1)*1000;
    ...
    j(i) = concatsniff_pdrug(1,i)~= 0 && concatsniff_pdrug(2,i)~= 0;
%     if j(i) == 1
%         plot([1 2 3],[normmaxmin_pdrug_fs(i) normmaxmin_cgpdrug_fs(i) normmaxmin_wshoutdrug_fs(i)],'--x');hold on;title('Sniff 1 dF (max(iodor)-min(ibase)) values normalized (Across Conditions)');xlabel('Condition (Predrug || CGP35348 || APV+NBQX');ylabel('{\Delta}F Normalized to Predrug'); 
%     end
end
...
concat_onsetpeaklat_wshout = vertcat(onsets_wshout,peaklats90_wshout,peaklat_wshout);
...
for i = 1:size(pdrug.data,1)
    ...
    normmean_wshoutdrug_ss(i) = (mean(results_trace_wshout(i,iodor2))-mean(results_trace_wshout(i,ibase2)))./(mean(results_trace(i,iodor2))-mean(results_trace(i,ibase2)));
j(i) = concatsniff_pdrug(1,i)~= 0 && concatsniff_pdrug(2,i)~= 0;
    if j(i) == 1
        plot([1 2 3],[normmean_pdrug_ss(i) normmean_cgpdrug_ss(i) normmean_wshoutdrug_ss(i)],'--x');hold on;title('Sniff 1 dF (mean(iodor)-mean(ibase)) values normalized (Across Conditions)');xlabel('Condition (Predrug || CGP35348 || Washout');ylabel('{\Delta}F Normalized to Predrug'); 
    end
end

%% bar plot
for i = 1:size(pdrug.data,1)
figure;bar(1,normmaxmin_pdrug_fs(i));hold on;bar(2,normmaxmin_cgpdrug_fs(i));bar(3,normmaxmin_wshoutdrug_fs(i));
end
...
for i = 1:size(pdrug.data,1)
    ...
    mean_wshoutdrug_ss(i) = (mean(results_trace_wshout(i,iodor2))-mean(results_trace_wshout(i,ibase2)));
j(i) = concatsniff_pdrug(1,i)~= 0 && concatsniff_pdrug(2,i)~= 0;
    if j(i) == 1
        plot([1 2 3],[mean_pdrug_ss(i) mean_cgpdrug_ss(i) mean_wshoutdrug_ss(i)],'--x');hold on;title('mean dF across odor duration(mean(iodor2)-mean(ibase2)) values non-normalized (Across Conditions)');xlabel('Condition (Predrug || CGP35348 || Washout');ylabel('{\Delta}F Normalized to Predrug'); 
    end
end
...
for i = 1:size(pdrug.data,1)
    ...
    maxmin_wshoutdrug_fs(i) = (max(results_trace_wshout(i,iodor))-min(results_trace_wshout(i,ibase)));
j(i) = concatsniff_pdrug(1,i)~= 0 && concatsniff_pdrug(2,i)~= 0;
    if j(i) == 1
        plot([1 2 3],[maxmin_pdrug_fs(i) maxmin_cgpdrug_fs(i) maxmin_wshoutdrug_fs(i)],'--x');hold on;title('Sniff 1 dF (max(iodor)-min(ibase)) values non-normalized (Across Conditions)');xlabel('Condition (Predrug || CGP35348 || Washout');ylabel('{\Delta}F Normalized to Predrug'); 
    end
end
...
mean_wshoutdrug_ss = mean_wshoutdrug_ss';
maxmin_wshoutdrug_fs = maxmin_wshoutdrug_fs';
...
figure;scatter(peaklats90_cgpdrug,peaklats90_wshout,'o','g','filled'); 
title('Normalized \DeltaF/F Unity Plot (CGP35348 vs WashoutAPV/NBQX)');
xlabel('CGP35348');
ylabel('Washout');
ylimit = ylim;
xlimit = xlim;
...
figure;scatter(onsets_cgp,onsets_wshout,'o','b','filled'); 
title('Normalized \DeltaF/F Unity Plot (CGP35348 vs Washout)');
xlabel('CGP35348');
ylabel('APV/NBQX');
ylimit = ylim;
xlimit = xlim;
...
clearvars -except pdrug cgpdrug wshoutdrug maxmin_pdrug_fs mean_pdrug_ss mean_cgpdrug_ss mean_wshoutdrug_ss maxmin_cgpdrug_fs maxmin_wshoutdrug_fs onsets_pdrug onsets_cgp onsets_wshout peaklats90_pdrug peaklats90_cgpdrug peaklats90_wshout results_trace results_trace_cgp results_trace_wshout results_ts results_ts_cgp results_ts_wshout


%% now transpose latency variables
onsets_pdrug = onsets_pdrug';
onsets_cgp = onsets_cgp';
onsets_wshout = onsets_wshout';
peaklats90_pdrug = peaklats90_pdrug';
peaklats90_cgpdrug = peaklats90_cgpdrug';
peaklats90_wshout = peaklats90_wshout';

for i=1:size(pdrug.data,1)
    normpratdrug_fs(i) = maxmin_pdrug_fs(i)/max(maxmin_pdrug_fs);
    normcgpratdrug_fs(i) = maxmin_cgpdrug_fs(i)/max(maxmin_pdrug_fs);
    normwshoutratdrug_fs(i) = maxmin_wshoutdrug_fs(i)/max(maxmin_pdrug_fs);
    normpratdrug_ss(i) = mean_pdrug_ss(i)/max(mean_pdrug_ss);
    normcgpratdrug_ss(i) = mean_cgpdrug_ss(i)/max(mean_pdrug_ss);
    normwshoutratdrug_ss(i) = mean_wshoutdrug_ss(i)/max(mean_pdrug_ss);
end

normpratdrug_fs = normpratdrug_fs';
normcgpratdrug_fs = normcgpratdrug_fs';
normwshoutratdrug_fs = normwshoutratdrug_fs';
normpratdrug_ss = normpratdrug_ss';
normcgpratdrug_ss = normcgpratdrug_ss';
normwshoutratdrug_ss = normwshoutratdrug_ss';
    
for i=1:size(pdrug.data,1)
fsrat_pdrugcgp(i) = normcgpratdrug_fs(i)/normpratdrug_fs(i);
fsrat_cgpwshout(i) = normwshoutratdrug_fs(i)/normcgpratdrug_fs(i);

ssrat_pdrugcgp(i) = normcgpratdrug_ss(i)/normpratdrug_ss(i);
ssrat_cgpwshout(i) = normwshoutratdrug_ss(i)/normcgpratdrug_ss(i);

onsetdiff_pdrugcgp(i) = onsets_cgp(i)-onsets_pdrug(i);
onsetdiff_cgpwshout(i) = onsets_wshout(i)-onsets_cgp(i);

peakdiff_pdrugcgp(i) = peaklats90_cgpdrug(i)-peaklats90_pdrug(i);
peakdiff_cgpwshout(i) = peaklats90_wshout(i)-peaklats90_cgpdrug(i);
end
...
medom_fs_cgpiwshout = median(fsrat_cgpwshout(:));
meanom_fs_cgpwshout = mean(fsrat_cgpwshout(:));
stdom_fs_cgpwshout = std(fsrat_cgpwshout(:));

medom_ss_cgpwshout = median(ssrat_cgpwshout(:));
meanom_ss_cgpwshout = mean(ssrat_cgpwshout(:));
stdom_ss_cgpwshout = std(ssrat_cgpwshout(:));
...
medom_onset_cgpwshout = median(onsetdiff_cgpwshout(:));
meanom_onset_cgpwshout = mean(onsetdiff_cgpwshout(:));
stdom_onset_cgpwshout = std(onsetdiff_cgpwshout(:));
...
medom_peaklat_cgpwshout = median(peakdiff_cgpwshout(:));
meanom_peaklat_cgpwshout = mean(peakdiff_cgpwshout(:));
stdom_peaklat_cgpwshout = std(peakdiff_cgpwshout(:));
...
figure;
for i=1:size(pdrug.data,1)
    plot([1.5 2.5],[fsrat_pdrugcgp(i) fsrat_cgpwshout(i)],'--x');
    title('1^s^t sniff amplitude ratio (Postdrug/Previous Condition)');
    xlabel('CGP35348/Predrug -------- Washout/CGP35348');
...
figure;
for i=1:size(pdrug.data,1)
    plot([1.5 2.5],[ssrat_pdrugcgp(i) ssrat_cgpwshout(i)],'--x');
...
figure;
for i=1:size(pdrug.data,1)
    plot([1.5 2.5],[onsetdiff_pdrugcgp(i) onsetdiff_cgpwshout(i)],'--x');
...
figure;
for i=1:size(pdrug.data,1)
    plot([1.5 2.5],[peakdiff_pdrugcgp(i) peakdiff_cgpwshout(i)],'--x');
