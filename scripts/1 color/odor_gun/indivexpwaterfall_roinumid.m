...
sorttotglusnfr_bothbanks = sortrows(totglusnfr_bothbanks,2);
expname = 'tbt57';
%% plot all glusnfr rois across experiments with labeled ROIs
figure;imagesc(sorttotglusnfr_bothbanks);title(['Significant Excitatory/Suppressive Non-normalized/Sorted Glomerular ROIs {\color{red}Exp ID:}', num2str(expname)]);ylabel('Glom-Odor Pairs based on ROI ID');xlabel('Frames');colormap(gca,nawhimar_auto);
c = colorbar;
...
figure;imagesc(normallglurois_bothbanks);title(['Significant Excitatory/Suppressive Normalized/Sorted Glomerular ROIs {\color{red}Exp ID:}', num2str(expname)]);ylabel('Glom-Odor Pairs based on ROI ID');xlabel('Time (s)');colormap(gca,nawhimar_auto);
c = colorbar;
c.Label.String = 'Norm .\DeltaF/F';
gray = [.9 .9 .9]; %odor onset and offset signal overlay 
patch([451 451 751 751],[1 1 1 1], gray);
...
nyticks = nyticks';
nytickslabel = sorttotglusnfr_bothbanks(:,2);
