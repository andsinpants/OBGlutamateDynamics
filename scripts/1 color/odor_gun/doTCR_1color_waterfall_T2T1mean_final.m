...
for r = 1:length(ex.myplotdata.avgfile.roi)
    ...
    for o = 1:length(ex.myplotdata.avgfile.roi(r).odor)
        ...
        exmt1 = mean(sup.myplotdata.avgfile.roi(r).odor(o).avgtrial.series(1:150));
        exmt2 = mean(sup.myplotdata.avgfile.roi(r).odor(o).avgtrial.series(300:end));
        exmdiff = exmt2-exmt1;
        ...
        supmt1 = mean(sup.myplotdata.avgfile.roi(r).odor(o).avgtrial.series(1:150));
        supmt2 = mean(sup.myplotdata.avgfile.roi(r).odor(o).avgtrial.series(300:end));
        supmdiff = supmt2-supmt1;
        ...
        elseif tmpmax>maxthresh %only excited
             results(r,odornum,1)=exmdiff;
             results_trace(index,1:length(exzts))=exzts;  %<<<<<<<<<<< extract time series (exc)
             results_ts(index,2)= odornum;
             results_ts(index,1)=r;
             results_ts(index,3)=exmdiff; results_ts(index,4)=0;
            if bplotbyodor
                figure(odornum); hold on;
        ...
        elseif tmpmin<minthresh %only suppressed
           results(r,odornum,2)=supmdiff; 
           results_trace(index,1:length(supzts))=exzts; %<<<<<<<<<<<<< extract time series (sup)
           results_ts(index,2)= odornum;
           results_ts(index,1)=r;
           results_ts(index,3)=0; results_ts(index,4)=supmdiff;
            if bplotbyodor
                figure(odornum); hold on;
