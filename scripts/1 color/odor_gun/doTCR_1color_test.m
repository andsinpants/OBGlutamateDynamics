...
results2 = nan(length(ch1_ex.myplotdata.avgfile.roi)*24,4);
...
%         results2(index,7)= rgcorrcoeff;
        if tmpmax>maxthresh && tmpmin<minthresh %plot both excitatory and suppressive
            ...
            results2(index,3)=tmpmax; results2(index,4)=tmpmin; results2(index,2)=odornum; results2(index,1)=r;
            if bplotbyodor
        ...
        elseif tmpmax>maxthresh %only excited
             ...
             results2(index,3)=tmpmax; results2(index,2)=odornum;results2(index,1)=r;
            if bplotbyodor
                figure(odornum); hold on;
        ...
        elseif tmpmin<minthresh %only suppressed
           ...
           results2(index,4)=tmpmin; results2(index,2)= odornum;results2(index,1)=r;
            if bplotbyodor
                figure(odornum); hold on;
...
%             results2(index,5)=tmpmax2; results2(index,6)=tmpmin2; results2(index,2)= odornum;results2(index,1)=r;
%             if bplotbyodor
...
%              results2(index,5)=tmpmax2; results2(index,2)= odornum;results2(index,1)=r;
%             if bplotbyodor
%                 figure(odornum); hold on;
%                 tmpcolor = 'g'; % use 'g' or myColors(r) here
%             else
...
%             results2(index,6)=tmpmin2; results2(index,2)= odornum;results2(index,1)=r;
%             if bplotbyodor
%                 figure(odornum); hold on;
%                 tmpcolor = 'b'; % use 'g' or myColors(r) here
%             else
...
delarray = isnan(results2(:,4));
da = find(delarray);
results3(da,:)=[];
results_sup = results3;

delarray = isnan(results2(:,3));
da = find(delarray);
results4(da,:)=[];
results_exc = results4;
%clear o 
%clearvars -except odornums r results2 results
%clear ch1_ex ch1_sup
%close all

% delarray = isnan(results2(:,7));
% da = find(delarray);
% results2(da,:)=[];
% 
% ch1ex_sig = isfinite(results2(:,3));
% ch1sup_sig = isfinite(results2(:,4));
% ch2ex_sig = isfinite(results2(:,5));
% ch2sup_sig = isfinite(results2(:,6));
% allchsig = horzcat(ch1ex_sig,ch1sup_sig,ch2ex_sig,ch2sup_sig);
% 
% ch1ex_vals = find(ch1ex_sig);
% ch1ex_svals = results2(ch1ex_vals,7);
% ch1sup_vals = find(ch1sup_sig);
% ch1sup_svals = results2(ch1sup_vals,7);
% ch2ex_vals = find(ch2ex_sig);
% ch2ex_svals = results2(ch2ex_vals,7);
% ch2sup_vals = find(ch2sup_sig);
% ch2sup_svals = results2(ch2sup_vals,7);
% 
% ch1and2ex_svals = find(allchsig(:,1)& allchsig(:,3));
% for i=1:length(ch1and2ex_svals)
% co_ex(i,:) = (results2(ch1and2ex_svals(i),:));
% end
% 
% ch1and2sup_svals = find(allchsig(:,2) & allchsig(:,4));
% for i=1:length(ch1and2sup_svals)
% co_sup(i,:) = (results2(ch1and2sup_svals(i),:));
% end
% 
% ch1and2no_svals = find(allchsig(:,1) | allchsig(:,2) | allchsig(:,3) | allchsig(:,4)); %this includes where both responses are active, need to filter out
% % sd_ch1_2_excandsup = setdiff(ch1and2sup_svals,ch1and2ex_svals);
% ch1or2_excorsup = zeros(length(ch1and2no_svals),7);
% for i=1:length(ch1and2no_svals)
% ch1or2_excorsup(i,:) = (results2(ch1and2no_svals(i),:));
% end
% 
% ch1orch2_nsvals = [];
% for i=1:length(allchsig)
%     tf(i,:) = allchsig(i,:);
%     if all(tf==0)
%         ch1orch2_nsvals(i,:) = results2(i,:);
%     else
% %     if all(tf)==1;
%         ch1orch2_nsvals(i,:) = [];
%     end
...
% 
% 
% mex_cc_sig = mean(co_ex(:,7));
% msup_cc_sig = mean(co_sup(:,7));
% mnoexorsup = mean(ch1or2_excorsup(:,7));
