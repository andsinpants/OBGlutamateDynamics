...
figure;imagesc(totglusnfr_bothbanks);title(['Significant Excitatory/Suppressive Unsorted Glomerular ROIs {\color{red}Odor ID:}', num2str(expname)]);ylabel('Glom-Odor Pairs');xlabel('Frames');colormap(gca,nawhimar_auto);
c = colorbar;
...
for j=1:size(allexcglurois_bothbanks,1) %% in progress
    normallexcglurois_bothbanks(j,:) = normalised_diff(allexcglurois_bothbanks(j,3:todoron:todoroff:end)); %normalize to odor only
end
...
for j=1:size(allsupglurois_bothbanks,1) %% in progress
    normallsupglurois_bothbanks(j,:) = normalised_diff(allsupglurois_bothbanks(j,3:todoron:todoroff:end)); %normalize to odor only
end
end
...
figure;imagesc(normallglusnfrrois);title(['Significant Excitatory/Suppressive Normalized Glomerular ROIs {\color{red}Odor ID:}', num2str(expname)]);ylabel('Glom-Odor Pairs');xlabel('Frames');colormap(gca,nawhimar_auto);
c = colorbar;
c.Label.String = '\DeltaF/F';
gray = [.9 .9 .9]; %odor onset and offset signal overlay 
patch([451 451 751 751],[1 1 1 1], gray);
...
hax = gca;
nyticks = [1:size(allexcglurois_bothbanks)];
nyticks = nyticks';
hax.YTick = [nyticks];
nytickslabel = allexcglurois_bothbanks(:,1);
hax.YTickLabel = [nytickslabel];
ytickcolor = unique(nytickslabel);
...
for k = 1:size(normallexcglurois_bothbanks,1)
   xfwhm(k) = fwhm(times,normallexcglurois_bothbanks(k,:));
end

% for k = 1:size(normallexcglurois_bothbanks,1)
%    xfwhm_odor(k) = fwhm(times,normallexcglurois_bothbanks(k,451:751));
% end

[peaktimes,peakind] = sort(xfwhm,'ascend');
normsorte = normallexcglurois_bothbanks(peakind,:);

figure;imagesc(normsorte);title(['Significant Excitatory Norm & Sorted Glomerular ROIs {\color{red}Odor ID:}', num2str(expname)]);ylabel('Glom-Odor Pairs');xlabel('Frames');colormap(gca,nawhimar_auto);
c = colorbar;
c.Label.String = '\DeltaF/F';
gray = [.9 .9 .9]; %odor onset and offset signal overlay 
patch([451 451 751 751],[1 1 1 1], gray);
...
for i=1
    if isempty(normallsupglurois_bothbanks)
        ...
        for k = 1:size(normallsupglurois_bothbanks,1)
            xfwhm(k) = fwhm(times,normallsupglurois_bothbanks(k,:));
        end
    end
[troughtimes,troughind] = sort(xfwhm,'descend');
normsorts = normallsupglurois_bothbanks(troughind,:);


figure;imagesc(normsorts);title(['Significant Suppressive Norm & Sorted Glomerular ROIs {\color{red}Odor ID:}', num2str(expname)]);ylabel('Glom-Odor Pairs');xlabel('Frames');colormap(gca,nawhimar_auto);
c = colorbar;
c.Label.String = '\DeltaF/F';
gray = [.9 .9 .9]; %odor onset and offset signal overlay 
patch([451 451 751 751],[1 1 1 1], gray);
...
figure;imagesc(normsortes);title(['Significant Excitatory/Suppressive Norm & Sorted Glomerular ROIs {\color{red}Odor ID:}', num2str(expname)]);ylabel('Glom-Odor Pairs');xlabel('Time (s)');colormap(gca,nawhimar_auto);
c = colorbar;
c.Label.String = '\DeltaF/F';
gray = [.9 .9 .9]; %odor onset and offset signal overlay 
patch([451 451 751 751],[1 1 1 1], gray);
